...
class gameBot(object):
    def __init__(self, force_game_fps=2, run_real_time=False, pass_quit_event=True):
        ...
        :param force_game_fps: Fixes the pygame timer functions so the ai will get input as if it were running at this fps
    ...
    def start(self):
        ...
        pygame.display.update = getAction(pygame.display.update, self._on_screen_update)
        pygame.event.get = getAction(pygame.event.get, self._on_event_get)
        pygame.time.Clock = getAction(pygame.time.Clock, self._on_time_clock)
        pygame.time.get_ticks = getAction(pygame.time.get_ticks, self.get_game_time_ms)
    ...
    def get_ms_per_frame(self):
        return 1000.0 / self.force_game_fps #each frame is 500 ms
